...
# These are the x-coordinates on the top and bottom surfaces of the airfoil where
# detailed boundary layer data is computed. It is made externally accessible
# (`nf.bl_x_points`) in case you want to use it dynamically.
def _compute_bl_x_points(n_points: int) -> np.ndarray:
    """Compute BL sample x locations as uniform cell centers in [0, 1]."""
    s = np.linspace(0.0, 1.0, n_points + 1)
    return (s[1:] + s[:-1]) / 2.0


N_BL_POINTS = 32
bl_x_points = _compute_bl_x_points(N_BL_POINTS)


# Here, we compute a small epsilon value, which is used later to clip values to
# suppress overflow. This avoids explicitly referring to float bit-widths.
_eps: float = 10 / np.finfo(np.array(1.0).dtype).max
_ln_eps: float = np.log(_eps)


def _sigmoid(x: float | np.ndarray) -> float | np.ndarray:
    """Numerically-stable logistic function, with clipping to suppress overflow."""
    x = np.clip(x, _ln_eps, -_ln_eps)
    return 1.0 / (1.0 + np.exp(-x))


def _sind(x: float | np.ndarray) -> float | np.ndarray:
    return np.sin(np.deg2rad(x))


def _cosd(x: float | np.ndarray) -> float | np.ndarray:
...
def _arr_length(x: float | np.ndarray) -> int:
    return int(np.asarray(x).size)


### Pre-load parameters with statistics about the training distribution
# Includes the mean, covariance, and inverse covariance of training data in the
# input latent space (25-dimensional).
...
def _squared_mahalanobis_distance(x: np.ndarray) -> np.ndarray:
    """
    Computes the squared Mahalanobis distance of a set of points from the
    training data.

    Args:
        x: Query points in the input latent space. Shape: (N_cases, N_inputs).
...
def get_aero_from_kulfan_parameters(
    ...
    for key in ("upper_weights", "lower_weights"):
        n_weights = _arr_length(kulfan_parameters[key])
        if n_weights != 8:
            raise ValueError(
                ...
                f"with a different CST resolution, refit it with 8 weights per side."
            )
    ...
    for row in input_rows:
        if _arr_length(row) > 1:
            if N_cases == 1:
                N_cases = _arr_length(row)
            else:
                if _arr_length(row) != N_cases:
                    raise ValueError(
                        f"The inputs to the neural network must all have the same length. (Conflicting lengths: {N_cases} and {_arr_length(row)})"
                    )
    ...
    upper_theta = ((10 ** y_fused[:, 6 : 6 + N]) - 0.1) / (
        np.abs(upper_bl_ue_over_vinf) * np.reshape(Re, (-1, 1))
    )
    upper_H = 2.6 * np.exp(y_fused[:, 6 + N : 6 + N * 2])

    lower_theta = ((10 ** y_fused[:, 6 + N * 3 : 6 + N * 4]) - 0.1) / (
        np.abs(lower_bl_ue_over_vinf) * np.reshape(Re, (-1, 1))
    )
    lower_H = 2.6 * np.exp(y_fused[:, 6 + N * 4 : 6 + N * 5])
