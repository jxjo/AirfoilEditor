...
class Flap_Setter_new (Flap_Definition):
    """ 
    ...
    def _set_flap (self):
        ...
        x_upper, y_upper = self._upper.x, self._upper.y
        x_lower, y_lower = self._lower.x, self._lower.y
        
            
        # 2. CALCULATE INTERPOLATED HINGE THICKNESS VALUES
        y_h_upper = np.interp(self.x_flap, x_upper, y_upper)
        y_h_lower = np.interp(self.x_flap, x_lower, y_lower)
        ...
        idx_u = np.searchsorted(x_upper, self.x_flap)
        x_upper = np.insert(x_upper, idx_u, self.x_flap)
        y_upper = np.insert(y_upper, idx_u, y_h_upper)
        
        idx_l = np.searchsorted(x_lower, self.x_flap)
        x_lower = np.insert(x_lower, idx_l, self.x_flap)
        y_lower = np.insert(y_lower, idx_l, y_h_lower)
        ...
        x_up_rot, y_up_rot = rotate_surface_segment(x_upper, y_upper, idx_u)
        x_lo_rot, y_lo_rot = rotate_surface_segment(x_lower, y_lower, idx_l)
