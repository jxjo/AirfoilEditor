...
class Polar_Set:
    ...
    def terminate_polar_generation_except_for (cls, airfoils):
        """ terminate all worker polar generation beside for 'airfoil' and Designs"""

        for polar_set in cls.instances:
            if (not polar_set.airfoil in airfoils) and (not polar_set.airfoil.usedAsDesign): 
    ...
    def load_or_generate_polars (self):
        ...
        nLoaded = self.load_polars ()

        if self.has_all_polars_loaded:                                  # all done  

            # finalize active tasks if everything is loaded 
            ...
            # did errors occur (Worker finished with error) 

            for task in self.polar_worker_tasks[:]:                     # make a copy to remove tasks 
                error = task.check_and_handle_error ()
                if error:
                    task.finalize() 
                    self.polar_worker_tasks.remove (task)
            return nLoaded
...
class Polar_Worker_Task (Polar_Definition):
    ...
    def check_and_handle_error (self) -> bool:
        """ 
        check for Worker error - if error
            - terminate Worker
            - mark polar as failed 
            - finalize 
        """
        if self._myWorker is None: return False

        if not self._myWorker.isRunning():
            if self._myWorker.finished_returncode:
