...
class X_Program:
    ...
    def __init__ (self, workingDir = None):
        ...
        self._pipe_error_lines  = []                        # errortext lines from stderr when finished
    ...
    def remove_tmp_file (self, file : str, iretry = 0):
        ...
        try: 
            ...
            if iretry < 5: 
                iretry +=1
                logger.debug (f"{self} Could not delete tmp input file '{file}' - Retry {iretry}")
                time.sleep (0.1)
                self.remove_tmp_file (file, iretry=iretry)
            else: 
                logger.error (f"Could not delete tmp input file '{file}' - {exc}")
    ...
    def finished_errortext (self): 
        ...
        # last try - get first non blank line of out 
        if self._pipe_error_lines and not text:
            for line in self._pipe_out_lines:
                if line: return line 
    ...
    async def _execute_async (self, args = [], workingDir = None, capture_output : bool =False): 
        """ asyncio execute self in workingDir  
        ...
        returncode  = 0 
        proc       = None                                   # subprocess process instance
        ...
        # prepare flags for subprocess

        if capture_output:
            stdout = asyncio.subprocess.PIPE                # output is piped to suppress window 
            ...
            stdout = None 
            stderr = asyncio.subprocess.PIPE                # Xoptfoil will write error to stderr
            if os.name == 'nt':
        ...
        logger.debug (f"==> create subprocess {self.name}: '{args}'")
        ...
        try: 
            ...
            proc = await asyncio.create_subprocess_exec(exe, *args, creationflags=flags, **startupinfo, 
                                                           stdout=stdout, stderr=stderr)

            pipe_result, pipe_error = await proc.communicate()
            returncode = proc.returncode

            logger.debug (f"==> subprocess ended {self.name} - returncode {returncode}")

            # finished - nice output strings 
            if pipe_error:  self._pipe_error_lines = pipe_error.decode().split ("\r\n")
            if capture_output and pipe_result: self._pipe_out_lines = pipe_result.decode().split ("\r\n")
...
class Worker (X_Program):
    ...
    def generate_polar (self, airfoil_pathFileName, 
        ...
        if os.path.isfile(airfoil_pathFileName): 
            ...
            returncode = self._execute_Worker ("polar", airfoil1=airfoil_pathFileName, 
            ...
            if run_async:
                ...
                polar_pathFileName = self.get_existingPolarFile (airfoil_pathFileName, 
                                                    polarType, re, ma , ncrit)
                
                self.remove_tmp_file (self._tmp_inpFile)                 #  remove tmpfile of worker
                self._tmp_inpFile = None
                return polar_pathFileName
            
        else: 
            raise Exception ("generatePolar - airfoil file %s doesn't exist" % airfoil_pathFileName )




    async def generate_polar_async (self, airfoil_pathFileName, 
                        polarType : str, 
                        re : float | list, 
                        ma : float | list, 
                        ncrit : float,
                        ...
                        nPoints=None):
        """ 
        Generate xfoil polar files for airfoil_pathFileName in directory of airfoil.
        """ 
        ...
        tmp_inpFile = self._generate_polar_inputFile (airfoil_pathFileName, 
        ...
        args, workingDir = self._build_worker_args ('polar',airfoil1=airfoil_pathFileName, inputfile=tmp_inpFile)

        # and execute worker 

        returncode = await self._execute_async (args, capture_output=True, workingDir=workingDir)

        self.remove_tmp_file (tmp_inpFile)         

        if returncode: 
            msg = "\n".join(self._pipe_error_lines)
            raise RuntimeError (f"{self.name} polar generation failed: {msg}")
...
# ---------- Private --------------------------------------
    ...
    def _generate_polar_inputFile (self, airfoilPathFileName, 
        ...
        airfoil_dir, _ = os.path.split(airfoilPathFileName)

        # create tmp input file 

        with NamedTemporaryFile(mode="w", delete=False, dir=airfoil_dir, prefix=TMP_INPUT_NAME, suffix=TMP_INPUT_EXT) as tmp:
...
if __name__ == "__main__":
    ...
    Worker().isReady (__file__, min_version='1.0.3')

    if Worker.ready:

        worker = Worker()     
        rc = asyncio.run (worker._execute_async (['-h'], capture_output=True))

        if rc: logger.error (f"{worker}: {worker.finished_errortext}")

        worker = Worker()

        try: 
            airfoil = 'E:\GitHub\AirfoilEditor_v3_dev\\test_airfoils\JX-GT-15v2.dat'
            asyncio.run ( worker.generate_polar_async (airfoil, 'T1', 700000, 0.0, 8.0))
