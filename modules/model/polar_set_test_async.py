...
class Polar_Task (Polar_Definition):
    ...
    def __init__(self, polar: Polar =None):
        ...
        self.nLoaded    = 0                             # n polars which were generated & loaded on finalize  
    ...
    async def run_polar_generation (self):
        """ 
        run async worker to generate self polars and load polar file into polar
        
        return number of new loaded polars 
        """
        ...
        try:
            logger.debug (f"{self} starting worker")

            await self._myWorker.generate_polar_async (self._airfoil_pathFileName, 
                                                    self._type, self._re, self._ma, self._ncrit, 
                                                    autoRange=self._autoRange, spec=self._specVar, 
                                                    valRange=self._valRange, nPoints=self._nPoints)

            logger.debug (f"{self} loading polars")
            ...
            logger.error (f"{self} - {exc}")
            for polar in self._polars:
                if not polar.isLoaded:
                    polar.set_error_reason ('Polar creation failed')
                    nLoaded += 1           

        self.nLoaded = nLoaded
        self._isRunning = False 
