...
class App_Main (QMainWindow):
    ...
    def __init__(self, airfoil_file, parent=None):
        ...
        if Worker.ready:
             watchdog = Polar_Watchdog_2 () 
             watchdog.sig_new_polars.connect         (self._diagram.on_new_polars)
             QTimer.singleShot (1000, lambda: asyncio.run (watchdog.loop()))
...
class Polar_Watchdog_2 (QObject):
    """ 
    Long running QThread 
        - loop current Polar_Tasks 
        - to check if new polars were generated 
            - signal new polars 
        - create new Polar Creation thread to generate new polars
    ...
    async def run (self) :
        ...
        n_new_polars = 0 
        polar_tasks = Polar_Task.refresh_get_instances () 

        async_tasks = []
        for polar_task in polar_tasks: 
            if not polar_task.isRunning():
                await polar_task.run_polar_generation()
                # async_tasks.append (polar_task.run_polar_generation()) 
        ...
        for polar_task in polar_tasks:                
            if polar_task.isCompleted():
                n_new_polars += polar_task.nLoaded
                polar_task.finalize()
